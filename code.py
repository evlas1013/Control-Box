...
b0 = digitalio.DigitalInOut(board.GP20)
...
while True:
    ...
    if b6.value and (lastPressed != 6 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_ALT, Keycode.SIX)
        lastPressed = 6
        currentDelay = 0
        continue

    if b7.value and (lastPressed != 7 or currentDelay > buttonDelay):
    ...
    if b10.value and (lastPressed != 10 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.ZERO)
        lastPressed = 10
        currentDelay = 0
        continue

    if b11.value and (lastPressed != 11 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.ONE)
        lastPressed = 11
        currentDelay = 0
        continue

    if b12.value and (lastPressed != 12 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.TWO)
        lastPressed = 12
        currentDelay = 0
        continue

    if b13.value and (lastPressed != 13 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.THREE)
        lastPressed = 13
        currentDelay = 0
        continue

    if b14.value and (lastPressed != 14 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.FOUR)
        lastPressed = 14
        currentDelay = 0
        continue

    if b15.value and (lastPressed != 15 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.FIVE)
        lastPressed = 15
        currentDelay = 0
        continue

    if b16.value and (lastPressed != 16 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.SIX)
        lastPressed = 16
        currentDelay = 0
        continue

    if b17.value and (lastPressed != 17 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.SEVEN)
        lastPressed = 17
        currentDelay = 0
        continue

    if b18.value and (lastPressed != 18 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.EIGHT)
        lastPressed = 18
        currentDelay = 0
        continue

    if b19.value and (lastPressed != 19 or currentDelay > buttonDelay):
        (k.send(Keycode.LEFT_CONTROL, Keycode.LEFT_SHIFT, Keycode.NINE)
        lastPressed = 19
        currentDelay = 0
        continue
